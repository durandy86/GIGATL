...
def findLatLonIndex(ds, lonValue, latValue):
    ...
    a = abs(ds.coords['x_rho'] - lonValue) + \
        abs(ds.coords['y_rho'] - latValue)
...
def rtree_xr(ds, var, xnew1d, ynew1d):
    ...
    # Find horizontal coordinates of the variable 
    x = [var.coords[s] for s in var.coords if "xi_" in s][0]
    y = [var.coords[s] for s in var.coords if "eta_" in s][0]
